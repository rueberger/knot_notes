...
That is, the \text{knot quandle} is the ``fundamental quandle'' of the knot complement, while the \textit{knot group} is the fundamental group of the knot complement. This quandle is a ``larger'' algebraic object than the group, and captures all information that the group doesn't recognize. For example, the knot group doesn't tell apart $K$ from $K^\ast$.
...
Discussion of Whitehead manifold\dots.
Time is of the essence! Work on your final projects!

\subsection{Classifying knots}

\subsubsection{$(p,q)$-torus knots}

Pros:

\begin{itemize}
  \item Easily drawn (Chris in particular likes this)
  \item Crossings are obvious
\end{itemize}
...
\begin{itemize}
  \item Often late to lab meeting
\end{itemize}

Here, $p$ refers to wrappings around the meridian and $q$ refers to the number of times the center hole is circled. In particular, a trefoil is a $(3,2)$-torus knot.
This construction is not an injective mapping into the space of knots: $(1,1), (0,1), (1, 0)$ are all the unknot. Also $(p,q) \cong (q, p)$

\subsubsection{Satellite knots}

Pros:

\begin{itemize}
  \item Gives simple procedure for producing knots from knots.
\end{itemize}

Cons:

\begin{itemize}
  \item Can be difficult to draw
\end{itemize}

To construct a satellite knot, take any knot $K$ and view it as sitting inside of some solid torus. Now take the solid torus and knot it into the tubular neighborhood of some other knot $J$. This warps $K$ into some possibly different knot, let's call it
$\Sat_{J}(K)$.

\begin{question}
  Is it possible that $c(\Sat_{J}(K)) \leq c(K)$? Does it depend on $J$?
\end{question}

Open problem: given a knot, figure out what type it falls into.

One result:


\begin{prop}
  Every composite knot is satellite knot. In particular $K_1 \# K_2 = \Sat_{K_2}(K_1)$. This is easy to see if you draw a diagram
\end{prop}

Whitehead doubling:
Take the unknot and rearrange it so the ends are `linked' and the two strands wrap around, then put that inside a torus. The satellite for this unknot is the whitehead double.

So it seems like Satellite knots are a pretty broad class, whereas $(p,q)$ torus knots are much more restricted.
...
This class dwarfs the other classes in size. There's some statement like, as the number of crossings grows, the probability of being a hyperbolic knot asymptotically approaches 1.
...
\begin{exercise}
  Learn Riemannian and Hyperbolic geometry, then show that the figure-8 knot is hyperbolic.
\end{exercise}


\begin{cor}
  Hyperbolic knots are prime.
\end{cor}
