...
\subsection{Axioms of the Jone's polynomial}

We denote the Jone's polynomial of a knot $K$ $V(K)$ or $V_K(A)$


\begin{itemize}
  \item $V_0 = 1$ normalization
  \item $V_K$ is isotopy-invariant
  \item $t^{-1} V_{\text{cross over}} - t  \V_{\text{cross under}} = (\sqrt{t} - \frac{1}{\sqrt{t}})V_{\text{don't cross at all}}$. This the Skein relation for the Jone's polynomial
\end{itemize}


\begin{ex}
  Put some twists in the unknot - what does that get you? You can easily compute the Jone's polynomial of the unlink.
...
\subsection{Uniqueness of Jone's polynomial}


\begin{thm}
Suppose you have some $I: \K = \set{\text{oriented links}} \to \Z[\sqrt{t}, \frac{1}{\sqrt{t}}]$.
If it satisfies the axioms of the Jone's polynomials stated above, then it is the Jone's polynomial.
\end{thm}


\begin{remark}
  The Skein relation does all of the heavy lifting here.

  It's trivial to satisfy the first relation, and what are you even doing if your map doesn't satisfy the second relation.
\end{remark}

\subsection{Just how sensitive is the Jone's polynomia}


\begin{question}
  Does $V$ `detect' the unknot? $V_K = 1 \implies K \cong 0$?
\end{question}
...
The Khovonav homology detects the unknot.

Thistelthwaite found a 2-component link $L$ such that $V_L = V_{0 0}$. Thus the Jone's polynomial is not
sensitive enough to detect the unlink.


Mutation is an aptly named operation that given $K$ outputs $M(K)$ produced by
drawing a box around a region of the knot and rotating it by $\pi$ in `different ways'.
$V_{M(K)} = V_K$. Uh oh.

\subsection{Other polynomials}

$K_-, K_+, K_0$ denote the undercrossing, overcrossing, and no crossing respectively
...
\begin{exercise}
  Show that $P(K \# J) = P(K) P(J)$ in two ways: by definition of $P$ and for $V$ using the definition of the bracket polynomials.
\end{exercise}


\begin{remark}
  $\nabla_K = 1 \iff G_k \defeq \pi_1(\R^3 - K) \text{ is perfect}$. Perfect means that $G$ is equal to it's commutator subgroup.
